...
async function main() {
  ...
  const buzzerQuestions = [
    { text: 'What is the largest mammal?', answer: 'Blue Whale', options: ['Blue Whale', 'Elephant', 'Giraffe', 'Whale Shark'] },
    { text: 'Who painted the Mona Lisa?', answer: 'Leonardo da Vinci', options: ['Leonardo da Vinci', 'Michelangelo', 'Raphael', 'Donatello'] },
    { text: 'What is the smallest prime number?', answer: '2', options: ['2', '1', '3', '0'] },
    { text: 'What is the capital of Japan?', answer: 'Tokyo', options: ['Tokyo', 'Kyoto', 'Osaka', 'Hiroshima'] },
  ];
  ...
  for (let i = 0; i < buzzerQuestions.length; i++) {
    await prisma.buzzerQuestion.create({
      data: {
        ...
        options: buzzerQuestions[i].options,
        quizId: quiz.id,
      },
    });
  }
  ...
  console.log('Teams: Team Alpha, Team Beta');
  console.log('Domains: Science, History, Geography, Sports (4 questions each)');
  console.log('Buzzer Questions: 4');
}
